ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "Scarab AA Gun Projectile"
ENT.Author = ""
ENT.Information = ""
ENT.Spawnable = false
...
function ENT:Initialize()
    if SERVER then
	 util.SpriteTrail( self, 0, Color(85,155,255,200), false, 128, 64, 2, 0.01, "effects/halo3/energy_trail" )
...
function ENT:ClientThink()
	...
	local smoke = emitter:Add("effects/halo_spv3/energy/electric_discharge", self:GetPos())
        smoke:SetGravity( Vector(math.Rand(-45, 45), math.Rand(-45, 45), math.Rand(-20, -25)) )
        smoke:SetVelocity( self:GetAngles():Forward() * -50 )
        smoke:SetDieTime( math.Rand(1.8,2.1) )
        smoke:SetStartAlpha( 255 )
        smoke:SetEndAlpha( 0 )
        smoke:SetStartSize( 75 )
        smoke:SetEndSize( 50 )
        smoke:SetRoll( math.Rand(-180, 180) )
        smoke:SetRollDelta( math.Rand(-0.2,0.2) )
        smoke:SetColor( 100, 165, 255 )
    ...
    if CLIENT then
        ...
        if self:IsValid() then
            cam.Start3D() -- Start the 3D function so we can draw onto the screen.
                render.SetMaterial( Material("effects/halo_spv3/flares/flare_soft0") ) -- Tell render what material we want, in this case the flash from the gravgun
                render.DrawSprite( self:GetPos(), math.random(100, 115), math.random(125, 150), Color(125, 185, 255) ) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
            cam.End3D()
        end
    end
end